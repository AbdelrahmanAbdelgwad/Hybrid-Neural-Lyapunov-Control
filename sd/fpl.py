...
def p_mean(l, p: float, slack=1e-15, default_val=tf.constant(0.0), axis=None):
    ...
    return (
        tf.cond(
            tf.reduce_prod(tf.shape(l)) == 0,  # condition if an empty array is fed in
            lambda: (
                tf.broadcast_to(default_val, tf_pop(tf.shape(l), axis))
                if axis
                else default_val
            ),
            lambda: tf.reduce_mean((l + slack) ** p, axis=axis),
        )
        ** (1.0 / p)
        - slack
    )
...
def build_piecewise(xy, val, clipped=False):
    ...
    for (prev_x, prev_y), (x, y) in items:
        final = tf.where(
            val < x, transform(val, prev_x, x, prev_y, y, clipped=clipped), final
        )
...
def smooth_constraint(
    ...
    return scale(
        tf.sigmoid(
            transform(
                x, from_low, from_high, sigmoid_low, sigmoid_high, clipped=clipped
            )
        )
    )
...
def tensor_to_str(tensor: tf.Tensor):
    return np.array2string(
        tensor.numpy().squeeze(), formatter={"float_kind": lambda x: f"{x:.2e}"}
    )


FPL = Union["Constraints", tf.Tensor]


def format_constraint(name_constraint: Tuple[str, FPL]):
    name, constraint = name_constraint
    constraint_str = (tensor_to_str if isinstance(constraint, tf.Tensor) else str)(
        constraint
    )
...
def move_toward_zero(x):
    # tweaked to be a good activity regularizer for tanh within the fpl framework
    def grad(dy):
        return -dy * x * x * x * 5.0
