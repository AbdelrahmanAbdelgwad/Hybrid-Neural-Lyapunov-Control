...
@keras.saving.register_keras_serializable(package="PendulumDynamics")
class PendulumDifferenceEq(keras.layers.Layer):
    """Pendulum difference equation as a serializable Keras layer."""
    ...
    def call(self, inputs):
        ...
        cos_th = tf.reshape(states[:, 0], (-1, 1))
        sin_th = tf.reshape(states[:, 1], (-1, 1))
        thdot = tf.reshape(states[:, 2], (-1, 1))
...
def pendulum_diff_Model():
    input_state = keras.Input(shape=(3,), name="state")
    input_action = keras.Input(shape=(1,), name="action")
    latent_input = keras.Input(shape=(0,), name="latent")
    inputs = layers.Concatenate()([input_state, input_action, latent_input])
    outputs = PendulumDifferenceEq()(inputs)
