...
def generator_def(env: ModelableEnv, hidden_sizes: list[int], latent_size: int):
    ...
    if not (
        isinstance(act_space, gym.spaces.Box) and isinstance(obs_space, gym.spaces.Box)
    ):
    ...
    normalized_action = (action_input - act_space.low) / (
        act_space.high - act_space.low
    )
    ...
    for hidden_size in hidden_sizes:
        dense = layers.Dense(
            ...
            kernel_regularizer=utils.PMean(),
        )(dense)

    dense = layers.Dense(state_size)(dense)
    outputs = (
        layers.Activation("sigmoid")(dense) * (obs_space.high - obs_space.low)
        + obs_space.low
    )
    model = keras.Model(
        inputs={"state": state_input, "action": action_input, "latent": latent_input},
        outputs=outputs,
...
def generate_fakes(generator, batch):
    latent_shape = tuple(batch["state"].shape[0:2]) + tuple(
        generator.input["latent"].shape[1:]
    )
    return {
        "state": batch["state"],
        "action": batch["action"],
        "next_state": generator_2d_batch(
            generator, {**batch, "latent": tf.random.normal(shape=latent_shape)}
        ),
    }


@tf.function
def direct_fpls(generator, batch, closeness):
    return fpl.Constraints(
        1.0,
        {
            "closeness": closeness(
                generate_fakes(generator, batch)["next_state"], batch["next_state"]
            )
...
@tf.function
def gan_fpls(generator, discriminator, real_batch, closeness_fpl):
    fakes = generate_fakes(generator, real_batch)
    fakes2 = generate_fakes(generator, real_batch)
    fooled = fpl.p_mean(discriminator(fakes), 2.0)
    fooled2 = fpl.p_mean(discriminator(fakes2), 2.0)
    ...
    real_fake_fpl = fpl.Constraints(
        0.0,
        {
            "real": fpl.p_mean(discriminator(real_batch), 0.0),
            "fake": fpl.p_mean(1 - discriminator(fakes), 0.0),
        },
    )
    fooled2_normalized = fpl.smooth_constraint(
        fooled2, 0.0, 0.5, 0.0, 0.97, starts_linear=True
    )
    generator_fpl = fpl.Constraints(
        2.0,
        {
            #         "fooled": fpl.p_mean(discriminator(fakes2),2.0)
            "fooled": fooled2_normalized,
            # "direct": direct_fpls(generator, real_batch, closeness_fpl)
            # "reg": tf.where(tf.stop_gradient(fpl.fpl_scalar(real_fake_fpl)) < 0.1, generator_regularizer, 1.0)
        },
    )
    generator_badness = 1.0 - tf.minimum(tf.stop_gradient(fooled2) * 2, 1.0)
    discriminator_fpl = fpl.Constraints(
        0.0,
        {
            "rf": real_fake_fpl,
            "reg": discriminator_regularizer * generator_badness
            + (1.0 - generator_badness),
            # linear interpolation between the regularizer and 1.0 decided by the amount fooled
        },
    )
    return generator_fpl, discriminator_fpl
...
def train_direct_step(env: ModelableEnv, generator, learning_rate):
    ...
    def gradient_step(batch):
        with tf.GradientTape() as gen_tape:
            generator_fpl = direct_fpls(generator, batch, env.closeness_fpl)
            generator_scalar = fpl.fpl_scalar(generator_fpl)
        ...
        return generator_scalar, generator_fpl
...
def train_GAN_step(env: ModelableEnv, generator, discriminator, learning_rate):
    ...
    def gradient_step(batch):
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            generator_fpl, discriminator_fpl = gan_fpls(
                generator, discriminator, batch, env.closeness_fpl
            )
            generator_scalar = fpl.fpl_scalar(generator_fpl)
            generator_loss = 1 - generator_scalar
            discriminator_scalar = fpl.fpl_scalar(discriminator_fpl)
...
def system_identify(
    ...
    env = modelable_env.make_modelable(
        gym.make(env_name)
    )  # make_modelable, create a loss for the env if it doesn't have one
    if load_saved:
        generator = tf.keras.models.load_model(load_saved)
    else:
        generator = generator_def(
            env, generator_hidden_sizes, latent_size
        )  # generator is a DNN for environment
    ...
    dataset = (
        tf.data.Dataset.from_generator(
            gather_mini_batch(env, episode_size), output_signature=dataset_spec
        )
        .shuffle(episode_size * 10)
        .batch(num_transitions, drop_remainder=True)
    ...
    data = (
        dataset.take(num_batches)
        .apply(tf.data.experimental.assert_cardinality(num_batches))
        .cache(f"caches/{env_name}_nb{num_batches}_e{episode_size}_b{batch_size}")
    )
    if gan:
    ...
    def save_checkpoint(epoch):
        try:
            for batch in validation_data:
                constraints = direct_fpls(generator, batch, env.closeness_fpl)
                print(f"{fpl.fpl_scalar(constraints):.2e}|{constraints}")
    ...
    utils.train_loop(
        [data] * epochs,
        trainer,
        every_n_seconds={"freq": save_freq, "callback": save_checkpoint},
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env_name", type=str, help="gym environment", default="Pendulum-v2"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of transition tuples representing the amount of values a discriminator has to work with in a batch",
    )
    ...
    parser.add_argument(
        "--gan",
        action="store_true",
        help="uses a GAN for learning a stochastic model of the transition function of the environment's observations",
    )
    parser.add_argument("--load_saved", type=str, default=None)
    parser.add_argument(
        "--generator_hidden_sizes", nargs="+", type=int, default=[100, 100]
    )
    parser.add_argument(
        "--discriminator_hidden_sizes", nargs="+", type=int, default=[300, 300]
    )
