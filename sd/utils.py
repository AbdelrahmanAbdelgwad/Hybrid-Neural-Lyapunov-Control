...
def load_checkpoint(path: Path):
    ...
    return keras.models.load_model(
        path.parent / "model.keras", custom_objects=custom_objects, safe_mode=False
    )
...
def train_loop(
    list_of_batches,
    train_step,
    every_n_seconds: Optional[FreqAndCallback] = None,
    end_of_epoch=None,
):
...
def mean_grad_size(grads):
    return sum(
        map(
            lambda grad_bundle: tf.norm(tf.abs(grad_bundle)),
            filter(lambda grad_bundle: not (grad_bundle is None), grads),
        )
    ) / len(grads)
