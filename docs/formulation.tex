...
expressed in \emph{Differentiable Fuzzy Logic} (FPL).  The controller is
...
expressed in FPL form (Section~\ref{sec:fpl}):
\[
...
a FPL constraint tree (Section~\ref{sec:loss_tree}).
...
% ===================================================================
\section{Differentiable Fuzzy Logic (FPL)}\label{sec:fpl}
% ===================================================================

FPL provides a framework for composing multiple objectives into a single
...
\subsection{Auxiliary FPL Primitives}
...
dimensions), the full loss is a nested FPL constraint tree.
...
\FOR{epoch $= 1$ \TO $E$}
    \FOR{each batch $\{(\x_i, \xstar_i)\}$}
        ...
        \STATE Evaluate FPL constraint tree $\mathcal{C}$
